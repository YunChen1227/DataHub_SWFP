...
//   - 源6 税票数据查询C 是【综合源】（一次同时给发票+税务）：两项请求时寻源器先试
//     综合源列表，它命中即可让证通四源与源5 全部 skipped；只拿回一维时缺的那维由
//     单维源补齐，且它绝不会在同一次请求里被调第二次；
//   - 收费标准按【实际查得的维度】判定，range 里的 dataScope/feeStandard 对下游明示。
//
// 全场景：综合源一次拿全 / 综合源半边 + 缺项补齐 / 综合源查无回落既有源 /
// scope=basic / dataType 单项 / 发票查无回落源5 / 税务查无按单发票计费 /
...
//
// 注意 mock_ctax 只对下面两个 CTAX 专属税号查得，其余一律 SYS404：源6 是综合源、
// 两项请求时排在最前，若它对既有场景税号也查得，证通/源5 的用例会全部被短路掉。
const (
	creditCodeNormal    = "92500233MA60R5KW8M" // 证通四产品 + 源5 均查得（源6 查无）
	...
	creditCodeCTaxAll   = "91110000CTAXALL001" // 源6 一次拿全两维 → 其余源全 skipped
	creditCodeCTaxTax   = "91110000CTAXTAX001" // 源6 只给税务 → 发票由证通发票源补齐
)
...
func main() {
	...
	rec.Check("两项皆得(源5 被短路, 源6 查无回落)", "errorCode=0 & body.code=001 & 源1-源4 ok & 源5 skipped & 源6 empty & feeStandard=both",
		r.ErrorCode == "0" && r.BodyCode == "001" && baseSourcesOK(r.Range) &&
			statusOf(r.Range, "源5") == "skipped" && statusOf(r.Range, "源6") == "empty" &&
			feeOf(r.Range) == "both" &&
			scopeOf(r.Range, "发票") && scopeOf(r.Range, "税务"), r.Raw)

	// 综合源主路径：一次调用满足两个维度，其余五个源一次都不调（省下来的就是钱）。
	r = harness.Query(version, appKey, harness.Secret, body(creditCodeCTaxAll), nil)
	rec.Check("综合源一次拿全两维", "errorCode=0 & body.code=001 & 源6 ok & 源1-源5 全 skipped & feeStandard=both",
		r.ErrorCode == "0" && r.BodyCode == "001" && statusOf(r.Range, "源6") == "ok" &&
			allSkipped(r.Range, "源1", "源2", "源3", "源4", "源5") &&
			feeOf(r.Range) == "both" && hasCTaxTaxData(r.Range) &&
			scopeOf(r.Range, "发票") && scopeOf(r.Range, "税务"), r.Raw)

	// 综合源只拿回税务：缺的发票维度由证通发票源补齐，税务源不再调用，综合源不重复调。
	r = harness.Query(version, appKey, harness.Secret, body(creditCodeCTaxTax), nil)
	rec.Check("综合源半边 + 缺项补齐", "errorCode=0 & body.code=001 & 源6 ok & 源1/源2 ok & 源3/源4 skipped & feeStandard=both",
		r.ErrorCode == "0" && r.BodyCode == "001" && statusOf(r.Range, "源6") == "ok" &&
			statusOf(r.Range, "源1") == "ok" && statusOf(r.Range, "源2") == "ok" &&
			allSkipped(r.Range, "源3", "源4") && feeOf(r.Range) == "both" &&
			hasCTaxTaxData(r.Range), r.Raw)
	...
	// 单维请求不走综合源阶段，但源6 仍在两个维度的候选列表里，且同优先级按【逻辑源
	// 总成本】升序——本地配置里它一次调用 400 分，比证通发票源的两次互补调用
	// (300+300) 便宜，故被先试；对 creditCodeNormal 它查无，于是回落证通。
	// 另一维度的源则完全不碰（省钱的第一现场）。
	r = harness.Query(version, appKey, harness.Secret, bodyWith(creditCodeNormal, "dataType", "invoice"), nil)
	rec.Check("dataType=invoice 只调发票源", "errorCode=0 & body.code=001 & 源6 先试即查无 & 源1 ok & 源3/源4 skipped & feeStandard=invoice",
		r.ErrorCode == "0" && r.BodyCode == "001" &&
			statusOf(r.Range, "源1") == "ok" && statusOf(r.Range, "源6") == "empty" &&
			allSkipped(r.Range, "源3", "源4") &&
			feeOf(r.Range) == "invoice" &&
			!scopeOf(r.Range, "税务"), r.Raw)

	r = harness.Query(version, appKey, harness.Secret, bodyWith(creditCodeNormal, "dataType", "tax"), nil)
	rec.Check("dataType=tax 只调税务源", "errorCode=0 & body.code=001 & 源6 先试即查无 & 源3 ok & 源1/源2/源5 skipped & feeStandard=tax",
		r.ErrorCode == "0" && r.BodyCode == "001" &&
			statusOf(r.Range, "源3") == "ok" && statusOf(r.Range, "源6") == "empty" &&
			allSkipped(r.Range, "源1", "源2", "源5") &&
			feeOf(r.Range) == "tax" &&
...
// hasCTaxTaxData 校验源6 的税务段已完成契约映射：sbsjList 有源6 条目，且利润表已按
// 项目行拍平（含 xmmc、不再有嵌套 data 节点）。
func hasCTaxTaxData(raw string) bool {
	c := parseContract(raw)
	if c == nil {
		return false
	}
	sbsj, ok := c.Tax["sbsjList"]["源6"]
	if !ok || !strings.Contains(string(sbsj), "sfzl") {
		return false
	}
	lrb, ok := c.Tax["lrbxxList"]["源6"]
	return ok && strings.Contains(string(lrb), "xmmc") && !strings.Contains(string(lrb), `"data"`)
}

// allSkipped 校验若干源本次全部未被调用（命中即停省下的那些钱）。
func allSkipped(raw string, sources ...string) bool {
	for _, s := range sources {
		if statusOf(raw, s) != "skipped" {
			return false
		}
	}
	return true
