...
func (o *QueryOrchestrator) Handle(ctx context.Context, signed *model.SignedRequest, cmd *model.QueryCommand) *model.QueryResponse {
	...
	// settleSrc 是逐源轨迹（含未调用的源），落 upstream_call 子表用于上游成本对账。
	var settleTok *quota.ReserveToken
	var settleDec *model.BillingDecision
	var settleSrc []model.SourceCall
	defer func() {
		rec.FoundData = rec.BusiCode == int(errs.BusiSuccess)
		rec.LatencyMs = lat()
		rec.CreatedAt = time.Now()
		o.submitBooks(settleTok, settleDec, settleSrc, rec, log)
	}()
	...
	settleTok, settleDec, settleSrc = out.settleTok, out.settleDec, out.sources
...
func (o *QueryOrchestrator) submitBooks(tok *quota.ReserveToken, dec *model.BillingDecision, src []model.SourceCall, rec *model.AuditRecord, log *slog.Logger) {
	t := bookTask{token: tok, decision: dec, sources: src, rec: rec}
	if o.books != nil {
		o.books.Submit(t)
		return
	}
	...
	defer cancel()
	if tok != nil && dec != nil {
		if err := o.quota.Settle(ctx, tok, t.settleInput()); err != nil {
